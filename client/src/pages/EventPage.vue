...
<template>
    <div v-if="towerEvent" class="container-fluid">
        <div class="p-5 pt-3 row justify-content-center">
            ...
            <div class="col-xxl-7 col-lg-9 col-sm-11 col-12">
                    ...
                    <div class="col-6">
                        <div class="row g-0">
                            <div class="col">
                                <div class="d-flex gap-3 align-items-center">
                                    ...
                                    <span class="px-2 rounded" :class="{ 'text-success bg-success-subtle': towerEvent.status == 'Active', 'text-danger bg-danger-subtle': towerEvent.status == 'Canceled', 'text-dark bg-dark-subtle': towerEvent.status == 'Sold Out', 'text-accent bg-accent-subtle': towerEvent.status == 'Expired' }">{{ towerEvent.status }}</span>
                                </div>
                        ...
                        <div class="mt-1 row bg-info rounded">
                            <div class="col p-4" v-if="comments.length != 0">
                                <div class="d-flex p-3 bg-page rounded shadow align-items-center gap-3" v-for="comment in comments" :key="comment.id">
                                    <img :src="comment.creator.picture" class="pfp" height="40">
                                    <div>
                                        <span class="fw-bold d-block">{{ comment.creator.name }}<span class="fw-normal ms-2 px-2 bg-accent-subtle text-accent rounded" v-if="comment.creatorId == towerEvent.creatorId">Host</span></span>
                                        {{ comment.body }}
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-3">
                        <div class="row g-1">
                            <div v-if="towerEvent.creatorId == account?.id && !towerEvent.isCanceled" class="col-12 bg-info rounded p-3 mb-3">
                                ...
                                <button class="btn btn-primary w-100 mb-2">Edit Event</button>
                                <button class="btn btn-danger w-100">Cancel Event</button>
                            </div>
                            <div class="col-12 bg-info rounded p-3 text-center">
                                <h5>Interested in going?</h5>
                                <p class="text-dark mb-3">Grab a ticket!<span v-if="tickets.find(x => x.accountId == account?.id) != null"><br>(You are attending this event)</span></p>
                                <button v-if="account" class="border-0 btn px-4 mb-2" :class="{ 'btn-accent' : towerEvent.status == 'Active', 'btn-dark' : towerEvent.status != 'Active' }" :disabled="towerEvent.status != 'Active'">{{ towerEvent.status == 'Active' ? 'Attend' : towerEvent.status }}</button>
                            ...
                            <div class="col-12 mt-3">
                                ...
                                <div v-else class="text-dark text-center bg-info rounded p-2">
